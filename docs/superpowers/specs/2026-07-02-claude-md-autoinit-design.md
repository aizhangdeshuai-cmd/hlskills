...
- 不内嵌项目详情(角色表/字段表/技术栈明细)——那些在 `.hl/knowledge/` 与 `.hl/memory/`,约束文件只放铁律+自检+指针,避免成为新漂移源。
...
- 不内嵌角色表/字段表/技术栈明细——那些在 `.hl/knowledge/` 与 `.hl/memory/`,约束文件只放指针,避免详情变化时约束文件也成为需同步的漂移源。
...
- **项目无 `.hl/knowledge/` 与 `.hl/memory/`**:约束文件仍创建(指针指向的目录可能暂不存在,指针本身有效;后续 hlkb/hllegacy 跑时会建目录)。
...
- **防漂移回归**:改一个字段后声明完成 → 确认 Agent 读自检段第 1 项 → 确认核对 .hl/knowledge/db/ 已更新。
