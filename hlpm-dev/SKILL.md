---
name: hlpm-dev
description: 多角色协作-开发段（15步）。从产品交付物验收到部署上线的全流程。开发段接手产品交付的 8 项必交物（来自 hlpm-product），执行版本一致性验证 + 拒收检查，然后进入开发主流程（Git 准备→开发→审查→联调→测试→审计→发布→部署）。Use when 产品交付物已就绪且开发即将启动，或调用 /hlpm-dev。
---
