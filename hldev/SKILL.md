---
name: hldev
description: 多角色协作-开发段(15步)。从产品交付物验收到部署上线的全流程。开发段接手产品交付的 9 项必交物(6 必出 + 3 条件出,来自 hlpm),执行版本一致性验证 + 拒收检查,然后进入开发主流程(Git 准备→开发→审查→联调→测试→审计→发布→部署)。Use when 产品交付物已就绪且开发即将启动,或调用 /hldev。
---
...
0. **★ 验证版本目录 + 9 项交付物 + 一致性矩阵**（`executor` 主导，阻塞点）
   - 接收来自 `hlpm` 步骤 12 的交付包
   - **:验证版本目录**
     - 确认交付物位于 `docs/vN/` 目录(而非散落在 `docs/` 根)
     - 扫描项目根目录:`ls -d docs/v*/ 2>/dev/null | sort -V`
     - 找到含 9 项交付物(6 必出:PRD/测试用例/验收/非功能/一致性/自检;3 条件出:设计稿/设计稿截图/竞品分析)的最新版本目录
     - **vN 标记文件检查**:
   ...
   - [ ] 6 必出齐全:prd / test-cases / acceptance-criteria / non-functional-requirements / consistency-matrix / handoff-self-check
...
13. **交付验证**（`verifier` 主导：创建完成标记）
    - 验收所有文档与功能完整性
    - 验证 15 项项目文档齐全（9 项产品段 + 6 项开发段补充，按 hlskills 主入口清单 / `hlpm/path-conventions.md`）
    - **:创建版本完成标记**:
...
| 步骤 | 关卡 | 触发后果 |
|------|------|---------|
| 0 | 9 项交付物 / 一致性矩阵 / 版本号 任一不符 | 拒收，禁入步骤 1 |
| 5 | 严重代码问题未修复 | 禁入联调 |
| 8 | 代码审查未通过 | 禁入测试 |
| 12 | 风险评分 < 50 | 禁入发布 |
...
开发段在产品段 9 项交付物(6 必出 + 3 条件出)基础上，补充以下 6 项（合计 15 项；编号与 `hlpm/path-conventions.md` §3.2 对齐）：
...
| 验证起点 | 第 0 阶段加载上下文 | 第 0 阶段验证 9 项交付物 + 一致性矩阵 |
| 拒收机制 | 无 | 开发可拒收 + 拒收原因 |
