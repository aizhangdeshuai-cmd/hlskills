...
12.8. **(可选) hlprd 业务方签字** — 用户在发布前需要业务方签字时, 调 `Skill hlprd "为 <项目名> 合成 v{N} 签字包"` 生成 `docs/v{N}/sign-off-package.docx`, 用户带着 .docx 找业务方签字. 业务方签后再回 hldev 继续 (本步骤插入 12.7 与 13 之间, 不影响 hldev 本身)
