...
| **多角色协作-产品段** | `hlpm-product` | `/hlpm-product` | **23步：需求→竞品（含参考咨询）→PRD评审会签→设计规范检查→保真度咨询→设计评审会签→用例评审会签→三项一致性终检→5项交付物打包。配套 hlpm-dev 使用** |
...
| **多角色协作**（产品 + 开发分工） | `hlpm-product` + `hlpm-dev` | 23 + 15 步分工，含重量评审会签、三项强同步、拒收机制 |
