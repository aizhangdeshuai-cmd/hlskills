---
name: hlskills
description: HL 项目开发技能合集总入口。包含新项目分析、新需求开发、Bug修复、旧项目分析、旧项目重构、浏览器实时QA、项目记忆、安装部署、Hooks配置、评审审查、设计规范、ADR、测试、发布、部署、Git、API设计、数据库迁移、错误处理、编码标准、无障碍共25项子技能。内嵌19个Agent。融合 gstack v1.44 浏览器引擎与结构化开发工作流（/investigate 调试 / /qa 测试 / /review 审查 / /cso 安全 / /canary 监控）。包含25项子技能。Use when 用户提到开发流程、PRD、需求评审、设计评审、Bug修复、代码审查、API设计、数据库迁移、发布部署、编码规范、无障碍等任何项目开发相关关键词。通过 /hlskills 调用，自动根据意图路由到对应子技能。
---
![hlpm-product](https://img.shields.io/badge/hlpm--product-v13-blue) ![review](https://img.shields.io/badge/review-3%20in%201-green)

> 当前版本: **v13** | 评审模式: **3 合 1(可选择)** | 原版 hlpm / hlpmnew 一字不改
# HL 项目开发技能合集
...
| 浏览器实时QA | `hlbrowse` | `/hlbrowse` | 70+命令：导航/交互/快照/截图/Cookie导入（集成 gstack v1.44 引擎） |
...
| **多角色协作**（产品 + 开发分工） | `hlpm-product` + `hlpm-dev` | 23 + 15 步分工,v11 新增角色边界铁律(产品不写代码)+ 6 条件路径(按规模和设计需求)+ 重量评审会签、三项强同步、拒收机制 |
...
所有规则的完整详细版本见 `project-dev-workflow` 技能（`/project-dev-workflow`）。
