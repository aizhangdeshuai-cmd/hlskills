...
- **评审模式(v22 起唯一)**: 选项 A 逐份立即评审——PRD(4.5)/设计(6b.6)/用例(8.5)各自输出后问用户"需要评审/不需要评审",选需要→5 角色 Agent 评审,选不需要→直接通过。**已取消**:分阶段评审(选项 B)、集中评审(选项 C)。
- **无实参**: 走第 0 步加载上下文 → 第 0.5 步阻塞询问规模/设计(评审模式已固定为逐份,不再询问)
