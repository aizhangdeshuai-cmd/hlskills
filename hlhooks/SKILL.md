...
- command 通过 stdin 接收 JSON,字段含 `tool_name` / `tool_input` / `tool_response`(PostToolUse 的工具输出字段名为 `tool_response`,不是 `tool_output`)
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '.tool_response' | grep -q 'error\\|Error\\|fatal\\|FATAL\\|permission denied' && echo '⚠️ 输出中发现错误关键词,请检查' || exit 0"
          }
        ]
      }
    ]
    ...
    "PostToolUse": [
      {
        ...
        "hooks": [
          {
            "type": "command",
            "command": "jq -r '.tool_response' | grep -q 'error\\|Error\\|fatal\\|FATAL\\|permission denied' && echo '⚠️ 输出中发现错误关键词' || exit 0"
          }
        ]
      }
    ]
