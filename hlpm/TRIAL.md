---
name: hlpm-trial
description: hlpm 试用说明(10 分钟跑通版)。用 1 个真实小需求,step-by-step 演示"启动 → 回答 3 问题 → 看到交付物 → 准备交接给开发"。Use when 用户问"试用一下 hlpm"/"能不能跑个 demo"/"先看个例子"。
---
...
```
启动 hlpm。我有个新需求:为主入口 SKILL.md 加 1 行 v18 状态徽章(顶部加一行显示当前版本 + 评审模式)
```
...
**Q3 评审模式**
- 选 A(逐份立即评审) — v18 默认,推荐;轻量需求三份都可选"不需要评审",跑得快
...
✓ 步骤 6a.2:保真度咨询 → 档一:高保真 HTML(生产态,默认)
✓ 步骤 6b:设计稿(高保真 HTML,URL 参数切态)
...
└── analysis/                           # 竞品分析(条件出:轻量模式可能空)
```

**快速检查**:
- [ ] 6 个必出文件都在(PRD / 用例 / 验收 / 非功能 / 矩阵 / 自检)
- [ ] `prd.md` 里有"v18 状态徽章"业务规则
...
# 在 main 分支加徽章
echo "" >> SKILL.md
echo "![version](https://img.shields.io/badge/hlpm--product-v18-blue)" >> SKILL.md
git add SKILL.md docs/v1/
git commit -m "v1: 加 v18 状态徽章"
```
...
按上面同样流程启动,但 Q1 选 B(标准)、Q3 选 A(逐份立即评审,每份选"需要评审")。
...
告诉 AI:
```
"这个需求实际比我想的复杂,升级到标准需求 + 逐份立即评审(每份选需要评审)"
```
