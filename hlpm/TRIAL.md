---
name: hlpm-trial
description: hlpm 试用说明(10 分钟跑通版)。用 1 个真实小需求,step-by-step 演示"启动 → 回答 2 问题 → 看到交付物 → 准备交接给开发"。Use when 用户问"试用一下 hlpm"/"能不能跑个 demo"/"先看个例子"。
---
...
在 IDE 中打开这个目录(`code ~/hlpm-trial` 或 `cursor ~/hlpm-trial`)。
...
```
启动 hlpm。我有个新需求:为主入口 SKILL.md 加 1 行 v25 状态徽章(顶部加一行显示当前版本 + 评审模式)
```

AI 会回应启动检查清单 + 1 个问题(评审模式固定逐份立即评审,只告知不提问)。

### Step 3: 回答 1 个问题(1 分钟)
...
同时 AI 会**告知**评审模式:逐份立即评审(v22 起唯一;v27 起不再提问)。三份产出都可选"不需要评审",跑得快
...
✓ 步骤 4.5:PRD 立即评审 → 用户选"不需要评审" → 直接通过
✓ 步骤 6a.1:设计规范检查 → 未发现,继续
✓ 步骤 6a.2:设计保真度规范(单文件 .html + bl-marker 钉点,无选档)
✓ 步骤 6b:设计稿(单文件 .html,带 bl-marker 钉点)
✓ 步骤 6b.6:设计立即评审 → 用户选"不需要评审" → 直接通过
✓ 步骤 8:测试用例
✓ 步骤 8.5:用例立即评审 → 用户选"不需要评审" → 直接通过
✓ 步骤 9.5:一致性终检(4 矩阵 ✅)
✓ 步骤 10/11:非功能需求 + 自检报告
✓ 步骤 12:打包交付
```
...
├── design/
│   ├── version-badge.html              # 设计稿(条件出:涉及 UI)
│   └── screenshot-version-badge.png    # 设计稿截图(条件出:6b.5 触发)
└── analysis/                           # 竞品分析(条件出)
```

**快速检查**:
- [ ] 5 个必出文件(PRD / 用例 / 非功能 / 矩阵(含验收标准) / 自检)都在
- [ ] `prd.md` 里有"v25 状态徽章"业务规则
- [ ] `test-cases.md` 里有 TC-001/TC-002 这种编号
- [ ] `consistency-matrix.md` 全部 ✅

### Step 6: 模拟开发接手(2 分钟)

虽然我们这次不实际改代码,但可以**模拟 hldev 步骤 0 的验证**:
...
- ✅ 启动产品段(0 阶段)
- ✅ 回答 2 个阻塞问题
- ✅ 看到 AI 自动跑约 13 步(逐份立即评审,三份均选不评审)
...
按上面同样流程启动,但在 4.5/6b.6/8.5 每份选"需要评审"。

**会多跑**:
- 完整竞品分析(2b/2c)
- 完整设计阶段(6a.1/6a.2/6b/6b.5/6b.6)
- 3 场逐份立即评审(4.5/6b.6/8.5,各 5 角色当场评)
- 4 个覆盖矩阵 + 不一致清单(完整跑)
...
记住几个关键点:
1. **1 个问题定路径** — 是否涉及设计(评审模式固定逐份立即评审,v27 起不再问)
2. **产品段不改代码** — 这是铁律
