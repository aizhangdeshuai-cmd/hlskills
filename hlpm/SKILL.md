...
0.5. **★ 需求规模与设计需求评估** — `analyst` 启动后,**一次性用 `AskUserQuestion` 合并问规模/设计/评审模式 3 个问题**(把这 3 个前置判断合并批量问,减少启动阶段打断;后续关卡该问仍会按需询问,如 0.6 版本目录、2a 竞品、6a.0 设计需求确认、4.5/6b.6/8.5 是否评审等):
