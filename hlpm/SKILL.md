---
name: hlpm
description: 多角色协作-产品段(分层多步流程)。从需求到交付物打包的全流程,覆盖加载上下文→竞品分析(含参考咨询)→PRD→设计规范检查→保真度咨询→设计→测试用例→三项一致性终检→打包交付。**唯一**评审模式:逐份立即评审(每份文档产出后当场 5 角色评,用户可选"需要/不需要";v22 起取消分阶段/集中评审)。与 hldev 配套使用:产品交付物评审通过后交接给开发。Use when 用户提出新需求且产品与开发是两个角色、需要显式交接,或调用 /hlpm。
---
...
> 用白话描述触发时机：**v18 起默认选项 A 改为"逐份立即评审"**;旧版分阶段/集中 v22 起取消,只留 A(逐份立即评审)。

- **选项 A: 逐份立即评审(v18 起默认,v22 起唯一,推荐)**
  - **适用**:所有规模(轻量/标准/复杂通用)
  - **机制**:
