...
本技能步骤 4.5/6b.6/8.5(逐份立即评审)及旧模式 5/7/9 出现的"5 方会签"是 markdown 描述——**实际执行时,同一个 LLM 在主 agent 上下文中按多个 role prompt 顺序扮演,产出意见**。**不是**真·多 agent 进程级对抗。
...
> **v18 评审模式变更**:默认改为"逐份立即评审"——每份文档(PRD/设计/用例)输出后**立即由 5 角色评审**(不攒到最后),要评就当场评、当场改。**取消 v17 的"待评审池 + 步骤 9a 联合评审"**(攒到最后统一评,反馈滞后)。旧"分阶段"(5/7/9)和旧"集中"(老 9a)保留为可选。

**如需真·多模型对抗**:需额外挂载多 LLM API key 配置,本流程不内置。

> 步骤 4.5/6b.6/8.5 及旧 5/7/9 直接引用本段(避免重复书写)。
...
> **步骤编号说明**:23 步用分层标签(0/0.5/0.6/1/2a-c/3/4/4.5/5/6a.0-2/6b/6b.5/6b.6/6b.5.1/7/8/8.5/9/9.5/10/11/12)而非线性 1→23,子步骤(如 6a.1)表示同一阶段内的细分。**v18 起 9a 已删除,评审改为 4.5/6b.6/8.5 三处逐份立即评审**。带标签的步骤合计 23 个;实际执行时按 0.5 步评估的规模/设计条件会跳过部分步骤。
...
- 选项 C: **集中评审**(v16 旧模式,保留可用)
  - 适用:轻量需求 / 迭代优化 / bug 修复
  - 流程:跳过中间 3 场会签, 在 9.5 一致性终检**之前**插入 1 场集中评审(老 9a)
...
| 标准 | 涉及 | 逐份立即评审 | 完整 23 步 | (无跳过) |
| 标准 | 不涉及 | 逐份立即评审 | 标准无设计 路径 | 整个第二阶段 |
| 复杂 | 涉及 | 逐份立即评审 | 完整 23 步 + 加严 | (无跳过;每份文档立即评) |
...
4. **PRD 编写**（`analyst` 生成内容 → 写入文件，**按功能点剖切(默认) + 6 附录**）
   ...
   - **🚨 4.5 PRD 立即评审(逐份立即评审模式专属)**
     - **触发条件**:0.5 步评审模式选 A"逐份立即评审"时执行;旧模式(B/C)跳过本步,直接进对应步骤 5/9a
...
5. **★ PRD 评审会签**（重量评审 1/3，阻塞点,仅"分阶段评审"模式执行）
   ...
   - **跳过条件**:选 B "集中评审" 时,本步骤+步骤 7+步骤 9 全部跳过,跳到集中评审(老 9a,选项 C 模式)
...
6a.2 **★ 设计保真度咨询**（阻塞点）
   ...
   - **🚨 两档共同硬性要求(可演示 + 可交互验证)**:
     ...
     - **交互脚本内联**:所有交互逻辑写在 HTML 内联 `<script>`,保证单文件可运行(参考 `examples/order-list-with-export-csv.html`)
     - **🚨 拆分规则(不设行数阈值,依复杂度自觉)**:
...
6b. **UI/UX 设计**（`designer`"已有页面微调"产出规则）
   ...
   **"全新页面"设计稿产出规则**（v17 补充,v18 修订: 拆分 + 演示态隔离）：
   ...
   6. **档二 demo 文件**(与生产文件 `.html` 物理隔离,见 6a.2 档二):每个交互元素挂 `data-prd`/`data-tc`/`data-state`/`data-matrix` + 🔗 角标侧边栏(见下方 5 区块扩展)。生产态 `.html` 文件**不挂**任何 `data-*` 属性、不含 🔗 角标、不含 DEMO-NOT-FOR-PROPDOM
   - **参考模板**:`examples/order-list-with-export-csv.html`(单文件、可交互、含状态切换)
   ...
   **"已有页面微调"设计稿产出规则**（v14 强制）：
   ...
   7. **🚨 设计稿输出后立即执行: 6b.5 截图**
      ...
      >
      > **目的**: 截图产物供 PRD 附录 D(原型截图引用)嵌入 + 步骤 7 设计评审查验交互效果。无截图 = 评审/交付看不到 UI 呈现。
      >
      ...
      - **不二次截图**: 步骤 7 (设计评审) 通过后不再重新截图。后续步骤复用本步产物。
   ...
   > **触发条件**：6a.2 选了"档二:高保真 + 逻辑说明活文档" → 3 区块扩为 5 区块；选"档一:高保真 HTML 原型"维持 3 区块原结构。
   >
...
7. **★ 设计评审会签**（重量评审 2/3，阻塞点,仅"分阶段评审"模式执行）
   - **触发条件**:仅当 0.5 步骤评审模式选 A "分阶段评审" 时执行本步骤
   - **跳过条件**:选 B "集中评审" 时,本步骤跳过,合并到集中评审(老 9a,选项 C 模式)
...
### 第三阶段.5:(v18 起已取消,评审在各文档产出后立即进行)

> **v18 变更**:原步骤 9a"联合评审"已删除。v18 默认模式"逐份立即评审"下,PRD/设计/用例在各自产出后(4.5/6b.6/8.5)**立即由 5 角色评审**,不再攒到最后统一评。
>
> - **默认模式(逐份立即评审)**:三份文档各自评完各自的 → 直接进 9.5 一致性终检
> - **旧集中评审模式(选项 C)**:仍在此位置(9.5 之前)插入 1 场 5 角色集中评审(评全部三份),规则见 0.5 步选项 C
> - **旧分阶段模式(选项 B)**:走步骤 5/7/9 三场独立会签,不经过此处
...
9.5. **★ 三项一致性终检**(阻塞点）
   ...
   - 9.5 返回后路径示例(**按评审模式分支**):
     - **逐份立即评审模式(v18 默认)**:
       ...
       - 若该文档当初"不需要评审"→ 返回对应步骤修改后,**该份仍无需评审**;但若修改牵连了其他文档,被牵连的文档按其原评审选择重评
     ...
     - **集中评审模式(旧)**(已走老 9a 一场,5/7/9 被跳过):
       - 不一致项在 PRD/设计/用例 → 返回对应步骤（4/6b/8）修改 → **集中评审(老 9a)必须整体重走 1 次**(强同步规则不可豁免已通过的评审)
...
12. **打包交付**
    - 输出 9 项交付物到版本目录 `docs/vN/`（6 必出 + 3 条件出。详见 :1068"交付文档清单"段）：
