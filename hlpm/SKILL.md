   ...
   - **15 字段功能点卡片模板**(每个 BL-N 一张,字段顺序固定):
     ...
     **{高/中/低/N/A}**({描述})
     ...
     - **🚨 用例截图(引言之后、`{N}.1 用例编号`之前)**: 每张卡片在编号前贴一张本功能的截图,让评审人先看到 UI 再读用例编号/规则。规则:
       ...
       3. **无图退化写地址**: 若 6b.5 未产出对应截图(如 hlbrowse 不可用),改为写图片地址文本 `截图: docs/v{N}/design/{page}.html(暂无截图占位)`,不留空
...
6b. **UI/UX 设计**（`designer`"已有页面微调"产出规则）
   ...
   **"已有页面微调"设计稿产出规则**（v14 强制）：
   ...
   7. **🚨 设计稿输出后立即执行: 6b.5 截图**
      ...
      - **失败处理**:
        - `hlbrowse` 不可用 → 跳过截图, PRD 附录 D 的 UI 部分标"暂无截图"占位
   ...
   - **🚨 6b.5.1 交互验证(v17 补充,6a.2"可交互"要求的落地关卡)**
     ...
     - **失败处理**:任一项不通过 → 返回 6b 修设计稿;`hlbrowse` 不可用 → 标"未做交互验证"并告知用户(不静默跳过)
   ...
   **`verifier` 二次验证**: 设计评审时(默认模式 6b.6 立即评审 / 旧分阶段模式步骤 7),`verifier` 用 `hlbrowse` 打开设计稿(档二用 `.demo.html`),**实际点击 1 个角标**(选最大块的 PR-XXX),**必须看到悬浮框展开 + 切到 tab 7 + 对应 logic-item 高亮**。否则评审不通过。
