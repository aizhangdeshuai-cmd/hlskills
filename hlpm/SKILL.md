...
> **程序化兜底:产品段每步后自检源代码** — 主 agent 在 1/2/3/4/5/6a/6b/7/8/9/9a/9.5/10/11 步骤完成后,**执行 `git status --short`**,确认未产生 `.ts/.js/.py/.java/.go/.vue/.tsx/.jsx` 等源代码变更(允许 .md / .html / .json / .yml 配置文件)。
> 命中源代码变更 → **回退该变更 + 报警**("🚨 角色边界铁律违反:产品段不应改源代码"),并报告用户。
> 例外: hlpm 阶段 6b 设计稿输出 `.html` 是允许的(本技能的"代码"指生产代码,非设计稿)。

### 版本基线表的 commit 流程

**触发位置**: PRD / 设计 / 测试用例任一基线版本 v1 登记到 `consistency-rules.md` 的版本基线表时,**必须 commit**。
...
> **失败处理**: `consistency-rules.md` 缺失 → 阻塞 9.5 通过;commit 失败(如 git 未初始化) → 警告但不阻塞(允许在交付物阶段手动补 commit)。
...
4. **PRD 编写**（`analyst` 生成内容 → 写入文件，7 大模块 + 1 可选段）
   - 业务逻辑：每个功能的完整业务规则，包含所有分支条件、边界情况、异常路径
   - 操作流程：每个用户操作的主流程 + 替代流程 + 异常流程
     - **流程图 (Mermaid 优先 + 文字双轨)**: 如流程复杂, 用 ```mermaid 块绘制, Agent 识别保留. 格式示例:
       ```mermaid
       flowchart TD
         A[用户操作] --> B{条件判断}
         B -->|满足| C[正常流程]
   ...
   - **🚨 原型截图引用(可选但推荐)**: 对涉及 UI/交互 的功能, PRD 中应附**原型截图路径**, 让评审人不用跳去看设计稿就能秒懂页面长啥样。格式:
     ```markdown
     ### 原型截图
     - 列表页: ![列表页原型](docs/v1/design/list-page.png)  → 源文件: `docs/v1/design/list-page.html`
     - 新增弹窗: ![新增弹窗](docs/v1/design/screenshot-3.png)  → 源文件: `docs/v1/design/add-modal.html`
     ```
     **规则**:
     1. **路径相对于 PRD 所在目录** (`docs/v1/`), 用相对路径 (例: `design/list-page.png`)
     2. **每张截图必须标注源 HTML** (`*.html`),评审人可点击看交互/动效
     3. **截图来源**: 步骤 6b.5 设计稿截图产物 (`screenshot[-N].png`) — 不重复截图,直接引用
     4. **不可用时**: 标 "🚧 暂无原型截图" + 链接到 `docs/v1/design/*.html` 源文件占位
     5. **跳过条件**: 本次需求 0.5 步选"不涉及设计" → 整个原型截图段不写(纯后端 PRD 不需要 UI 截图)
   ...
   - **🚨 模糊字眼 grep 自检(硬性)**: 步骤 4 写完后, 主 agent 执行:
     ...
     豁免清单(允许出现): "等"作为"等等"语气的引号内文字 / "TBD"在显式标注的"待办"段中(且附 owner + 截止日期)
