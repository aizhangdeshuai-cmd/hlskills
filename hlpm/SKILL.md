...
4a. **★ PRD 大纲(功能点清单 + 优先级初判,阻塞点)**
   ...
   - **执行**:`analyst` 基于步骤 1-3.5 产出,输出大纲:
     ...
     - **附录预留**:列出将用的附录(A 状态机/B 权限/C 埋点/D 原型/E 决策/F 差异/G 业务流程总图)
...
4b. **PRD 详细编写**（`analyst` 生成内容 → 写入文件，**按功能点剖切(默认) + 7 附录,基于 4a 大纲**）
   ...
   - **🚨 4.5 PRD 立即评审(逐份立即评审模式专属)**
     ...
     - **立即评审执行**(选"需要评审"时):
       ...
       - **硬性检查(v26)**: ① §0 含 Non-Goals(本期明确不做+理由);② 每条 AC 可验证(无"正常工作/响应良好/体验流畅/基本可用"式表述);③ {N}.15 未决问题每条带 owner + 期望解决时间,或显式标"本期不做";④ §0 假设清单逐条核对成立(有 [Assumption] 项时)。任一不满足 → 不通过
   ...
   **🚨 BL 钉点接入规范(v23 强制,bl-marker 系统) — 让设计稿自解释 PRD**
      ...
      - **接入步骤**(以新设计稿 `docs/v{N}/design/dashboard.html` 为例):
        ...
        5. **在 UI 元素上钉位**(父元素需 `position:relative`):
           ...
           ```
        6. **填 REQUIREMENTS 数据**: 改 `bl-marker.js` 顶部的 `REQUIREMENTS` 对象,每个 BL 含 `title` + `sections[]`。**内容摘自 `docs/v{N}/prd.md` §N 原文,不二次解读**。v28 起规则:
...
8. **验收用例编写**（`test-engineer`，含 E2E 业务路径骨架）
   ...
   - 用例必须能反推 PRD 验收标准
   - **沉淀**:本步产出的验收用例,由 `hldev` 步骤 10 沉淀到 `knowledge/tests/{module}.md`(总测试库,跨版本累计维护,标注引入版本)。本步产出的 `docs/{ver}/test-cases.md` 是版本快照(验收用例,固定不变);hldev 步骤 10 会把白盒用例追加到该快照,并同步沉淀到 `knowledge/tests/` 总库
...
12. **打包交付**
    - 输出 8 项交付物到版本目录 `docs/vN/`（5 必出 + 3 条件出。详见下方"交付文档清单（产品段 8 项）"段）：
...
| 2 | PRD 文档 | `docs/{ver}/prd.md` | `analyst` 生成 → 写入文件 | 4a/4b |
...
- PRD 功能点增删 / 衔接关系变更 → **附录 G 业务流程总图同步重画**(Mermaid + 文字双轨)
...
| 2 | PRD 文档 | `docs/{ver}/prd.md` | **必出** | 4a/4b |
...
| 7 | 一致性矩阵(含验收标准) | `docs/{ver}/consistency-matrix.md` | **必出** | 9.5 |
...
>
> 1. 验证 `docs/vN/` 目录存在 + 8 项交付物中的必出项文件非空(5 必出;竞品/设计稿/截图按条件)
