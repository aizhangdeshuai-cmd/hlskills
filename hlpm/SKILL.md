...
4. **PRD 编写**（`analyst` 生成内容 → 写入文件，**按功能点剖切(默认) + 6 附录**）
   ...
   - **14 字段功能点卡片模板**(每个 BL-N 一张,字段顺序固定):
     ...
     ![BL-{N} 用例截图](design/screenshot-{N}.png)
     ...
     **{高/中/低/N/A}**({描述})
     ...
     - **🚨 用例截图(引言之后、`{N}.1 用例编号`之前)**: 每张卡片在编号前贴一张本功能的截图,让评审人先看到 UI 再读用例编号/规则。规则:
       1. **优先嵌入真图**: 用 Markdown `![BL-{N} 用例截图](design/screenshot-{N}.png)` 直接显示(路径相对 PRD 所在的 `docs/{ver}/` 目录)
       2. **来源**: 复用步骤 6b.5 已生成的 `docs/{ver}/design/screenshot[-N].png`,**不重复截图**;卡片 BL-{N} 对应哪张图,由 designer/analyst 按页面归属映射(一个页面覆盖多个 BL 时可复用同一张)
...
8. **测试用例编写**（`test-engineer`，含 E2E）
   ...
   - **🚨 每条 TC 编号前贴截图**: 在每条用例的 `TC-XXX` 编号**之前**贴一张该用例对应界面/状态的截图,让评审人先看到 UI 再读用例步骤。规则:
     1. **优先嵌入真图**: 用 Markdown `![TC-XXX 截图](design/screenshot-{N}.png)` 直接显示(路径相对 `test-cases.md` 所在的 `docs/{ver}/` 目录)
     2. **来源**: 复用步骤 6b.5 已生成的 `docs/{ver}/design/screenshot[-N].png`,**不重复截图**;TC 粒度更细时(如某弹窗/某异常态),指向最贴近该用例状态的那张截图,同一状态多条 TC 可复用同一张
     3. **无图退化写地址**: 若 6b.5 未产出对应截图,改为写地址文本 `截图: docs/v{N}/design/{page}.html(暂无截图占位)`,不留空
     4. **跳过条件**: 0.5 步选"不涉及设计"(纯后端/接口 TC)→ 本截图块不写
     - **TC 条目模板(截图在编号前)**:
       ```
       ![TC-045 截图](design/screenshot-1.png)
