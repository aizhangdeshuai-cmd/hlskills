# 8 项产品交付物打包模板（产品段 → 开发段）

> 属于 `hlpm` 技能第 11-12 步骤。
> 产品段完成后，按本模板打包 8 项交付物(5 必出 + 3 条件出)到版本目录 `knowledge/doc/vN/`，交付给开发段（`hldev`）。

## 交付清单（8 项，5 必出 + 3 条件出）

| # | 交付物 | 路径模板 | 必出/条件 | 负责 Agent |
|---|--------|---------|-----------|-----------|
| 1 | 竞品分析报告 | `knowledge/doc/{ver}/analysis/competitive-analysis.md` | 条件(新项目/品类调研) | `analyst` 生成 → 写入文件 |
| 2 | PRD 文档 | `knowledge/doc/{ver}/prd.md` | **必出** | `analyst` 生成 → 写入文件 |
| 3 | 设计稿 | `knowledge/doc/{ver}/design/*.html` | 条件(涉及 UI) | `designer` |
| 4 | 设计稿截图 | `knowledge/doc/{ver}/design/screenshot-<page-name>.png` | 条件(6b.5 触发) | `designer` 调 `hlbrowse` |
| 5 | 测试用例 | `knowledge/doc/{ver}/test-cases.md` | **必出** | `test-engineer` |
| 6 | 非功能需求 | `knowledge/doc/{ver}/non-functional-requirements.md` | **必出** | `analyst` 生成 → 写入文件 |
| 7 | 一致性矩阵(含验收标准) | `knowledge/doc/{ver}/consistency-matrix.md` | **必出**（v5） | `analyst` 生成 + `verifier` 验证 → 写入文件 |
| 8 | **自检报告** | `knowledge/doc/{ver}/handoff-self-check.md` | **必出**（v6） | `analyst` 生成 → 写入文件 |

---

## 版本号管理（v5 强制）

**PRD / 设计稿 / 测试用例 三项必须版本号一致**才能交付：

```
当前交付版本：vX（X ≥ 1）
- PRD vX
- 设计 vX
- 用例 vX
```

版本变更触发条件：
- 任一项评审驳回导致内容修改 → 该项版本号 +1
- 触发【强同步】→ 其他两项必须同步 +1，三项版本号重新对齐
- 一致性矩阵每次重新生成时记录当前版本号

---

## 自检清单（产品段 11 步执行）

### 1. PRD 自检

- [ ] §0 + BL-N 卡片 15 字段齐全（涉及 UI 的卡片另需含 9.5 字段与控件行为表, v30) + 附录 A-H 按需齐全（附录 G/H 豁免需显式标注）
- [ ] 跨 BL 业务流程总图（附录 G）：Mermaid + 文字双轨齐全（无跨 BL 流转可豁免：PRD 仅 1 个 BL 或各 BL 相互独立；豁免需在 PRD 显式标注）
- [ ] 无模糊字眼（"暂定" / "待确认" / "或者" / "大致" / "大概" / "类似" / "TBD" / "TODO" / "FIXME" / "XXX" / 行尾省略号 / 行尾"等"）
- [ ] 字段级定义（类型 / 必填 / 校验 / 默认值 / 示例）
- [ ] 3 角色抽问测试通过（设计师 / 开发 / 测试无疑问）
- [ ] **版本号 vX**
- [ ] PRD 评审会签记录（5 方签字，意见表附后；或本次选"不需要评审"，无签字记录，留痕即可）

### 2. 设计稿自检

- [ ] **项目设计规范遵循**（v4 硬性项）：
  - 引用 6a.1 找到的规范文件路径
  - 规范遵循清单全部勾选（字体 / 色彩 / 间距 / 圆角 / 阴影 / 组件 / 动效）
  - 与现有页面并排对比（截图对照），逐项核对
- [ ] **设计稿形态记录**（v23 起）：单文件 .html 带 bl-marker 钉点，由 6a.2 步骤确定
- [ ] 符合单文件 bl-marker 形态的全部特征：引用 bl-marker.css/.js、钉点默认显示、含抽屉 `<aside id="bl-drawer">` / 悬浮开关 `<button id="bl-toggle">` / REQUIREMENTS 对象、URL 参数切态（`?bl-test=1` 调试面板）；开发者不写钉点部分
- [ ] 所有页面覆盖（列表 / 详情 / 表单 / 设置 / 登录注册 / 404 / 空状态 / 加载态）
- [ ] 每页 5 态（默认 / hover / 空 / 错 / 加载 + 边界态）
- [ ] 移动 / 平板 / 桌面三屏布局
- [ ] 无障碍 WCAG 2.2 AA 通过
- [ ] **版本号 vX，与 PRD 版本号一致**
- [ ] 设计评审会签记录（5 方签字；或本次选"不需要评审"，无签字记录，留痕即可）

### 3. 测试用例自检

- [ ] 覆盖主流程 + 替代流程 + 异常流程
- [ ] 边界值用例
- [ ] 权限分支用例
- [ ] E2E 场景
- [ ] 用例可反推 PRD 验收标准
- [ ] **业务规则 / 状态机 / 权限 三个维度均覆盖**
- [ ] **版本号 vX，与 PRD / 设计版本号一致**
- [ ] 用例评审会签记录（逐份立即评审 5 方签字；或本次选"不需要评审"，无签字记录，留痕即可）

### 4. 验收标准自检（已并入一致性矩阵 §1）

> 验收标准不再独立成文档,其内容(AC ID / 对应 PRD 规则 / 对应测试用例 / 验收方法)已并入一致性矩阵 §1 业务规则覆盖矩阵。本节改为核对矩阵 §1 填写质量。

- [ ] 一致性矩阵 §1 每条业务规则已编号 `AC-NNN`
- [ ] §1 每条有"验收方法"列(可观察的验收手段,非歧义措辞)
- [ ] §1 与 PRD 业务规则一一对应
- [ ] §1 状态列无 ❌/⚠️(全部 ✅)

### 5. 非功能需求自检

- [ ] 性能：具体 QPS / 响应时间
- [ ] 安全：加密 / 鉴权 / 审计要求
- [ ] 兼容性：浏览器 / 设备 / 系统列表

### 6. 一致性矩阵自检(单独交付物，与 consistency-rules.md 4 子表对齐）

- [ ] 业务规则覆盖矩阵完整（每条 PRD 业务规则对应到设计页面 + 测试用例）
- [ ] 状态机覆盖矩阵完整（每个状态对应到设计状态页 + 测试用例）
- [ ] 权限覆盖矩阵完整（每条权限规则对应到设计权限点 + 测试用例）
- [ ] **非功能需求覆盖矩阵完整**：
  - [ ] 性能覆盖矩阵（4.1）：每条 PRD 性能指标对应到 UI 实现策略 + 测试验证方法
  - [ ] 安全覆盖矩阵（4.2）：每条 PRD 安全要求对应到 UI 实现策略 + 测试验证方法
  - [ ] 兼容覆盖矩阵（4.3）：每条 PRD 兼容要求对应到 UI 实现策略 + 测试验证方法
- [ ] 不一致清单为空或已注明处理方式
- [ ] 全部 ✅ 通过（或不一致项已闭环）
- [ ] 记录当前版本号（vX）
- [ ] **代码实现追踪矩阵自检（§6.5，与 `knowledge/doc/{ver}/consistency-matrix.md` §5 矩阵对齐）**：
  - [ ] 第 5 矩阵存在，列齐全（# / PRD 条款 / 设计页面 / 测试用例 / 代码提交 SHA / 实现状态 / 偏离说明）
  - [ ] 每行已填代码提交 SHA（开发完成后回填）
  - [ ] 实现状态全部为 ✅ / N/A（本节关注矩阵填写完整性；❌/⚠️/➕ 闭环情况见 §9 拒收单）
  - [ ] 偏离说明字段对 ⚠️ 项完整记录偏离原因 + 修复计划
  - [ ] MVP 豁免（如适用）：豁免理由已记录在不一致清单
  - [ ] 与 PRD / 设计 / 用例版本号一致（vX）

---

## 自检报告模板（步骤 11 输出）

> 步骤 11 自检报告 = 本模板的勾选结果快照 + 签字方 + 当前版本号
> **防漂移设计**：自检报告**直接复制**"## 自检清单"（第 40-109 行）的 7 项勾选结果（含 §6.5），不另行维护清单
> 与"## 交付包总览"的关系：自检报告 = 自检结果；总览 = 交付摘要。两者**不重叠**。

`knowledge/doc/{ver}/handoff-self-check.md` 应包含：

```markdown
## 交付物自检报告

**项目**：
**自检日期**：
**自检人**（analyst）：
**对应交付包版本**：vX（与 PRD / 设计 / 用例 一致）

### 1. PRD 自检结果
- [✅/❌] §0 + BL-N 卡片 15 字段齐全（涉及 UI 的卡片另需含 9.5 字段与控件行为表, v30) + 附录 A-H 按需齐全（附录 G/H 豁免需显式标注）
- [✅/❌] 跨 BL 业务流程总图（附录 G）双轨齐全（无跨 BL 流转可豁免：PRD 仅 1 个 BL 或各 BL 相互独立；豁免需显式标注）
- [✅/❌] 无模糊字眼（"暂定" / "待确认" / "或者" / "大致" / "大概" / "类似" / "TBD" / "TODO" / "FIXME" / "XXX" / 行尾省略号 / 行尾"等"）
- [✅/❌] 字段级定义（类型 / 必填 / 校验 / 默认值 / 示例）
- [✅/❌] 3 角色抽问测试通过
- [✅/❌] 版本号 vX
- [✅/❌] PRD 评审会签记录（5 方签字；或本次选"不需要评审"，无签字记录，留痕即可）

### 2. 设计稿自检结果
- [✅/❌] 项目设计规范遵循（路径：____）
- [✅/❌] 设计稿形态记录：单文件 .html + bl-marker 钉点（v23）
- [✅/❌] 符合单文件 bl-marker 形态的全部特征（bl-marker.css/.js、抽屉 / 悬浮开关 / REQUIREMENTS 对象、URL 参数切态）
- [✅/❌] 所有页面覆盖
- [✅/❌] 每页 5 态
- [✅/❌] 三屏布局
- [✅/❌] 无障碍 WCAG 2.2 AA
- [✅/❌] 版本号 vX，与 PRD 一致
- [✅/❌] 设计评审会签记录（5 方签字；或本次选"不需要评审"，无签字记录，留痕即可）

### 3. 测试用例自检结果
- [✅/❌] 覆盖主/替代/异常流程
- [✅/❌] 边界值用例 / 权限分支用例 / E2E 场景
- [✅/❌] 业务规则 / 状态机 / 权限 三维覆盖
- [✅/❌] 版本号 vX，与 PRD / 设计 一致
- [✅/❌] 用例评审会签记录（逐份立即评审 5 方签字；或本次选"不需要评审"，无签字记录，留痕即可）

### 4. 验收标准自检结果（已并入一致性矩阵 §1）
- [✅/❌] 矩阵 §1 每条业务规则已编号 `AC-NNN`
- [✅/❌] §1 每条有"验收方法"列(可观察手段,非歧义)
- [✅/❌] §1 与 PRD 业务规则一一对应
- [✅/❌] §1 状态列无 ❌/⚠️

### 5. 非功能需求自检结果
- [✅/❌] 性能：具体 QPS / 响应时间
- [✅/❌] 安全：加密 / 鉴权 / 审计要求
- [✅/❌] 兼容性：浏览器 / 设备 / 系统列表

### 6. 一致性矩阵自检结果
- [✅/❌] 业务规则覆盖矩阵完整
- [✅/❌] 状态机覆盖矩阵完整
- [✅/❌] 权限覆盖矩阵完整
- [✅/❌] 非功能需求覆盖矩阵完整
  - [✅/❌] 性能覆盖矩阵（4.1）
  - [✅/❌] 安全覆盖矩阵（4.2）
  - [✅/❌] 兼容覆盖矩阵（4.3）
- [✅/❌] 不一致清单为空或已闭环
- [✅/❌] 全部 ✅ 通过

### 6.5 代码实现追踪矩阵自检结果(与 `knowledge/doc/{ver}/consistency-matrix.md` §5 对齐）
- [✅/❌] 第 5 矩阵存在，列齐全
- [✅/❌] 每行已填代码提交 SHA
- [✅/❌] 实现状态全部为 ✅ / N/A（矩阵填写完整性）
- [✅/❌] §9 拒收单无未闭环偏离项（对应 handoff-rejection.md）
- [✅/❌] 偏离说明完整
- [✅/❌/N/A] MVP 豁免（如适用）已记录理由
- [✅/❌] 与 PRD / 设计 / 用例版本号一致

### 7. 版本对齐
- PRD vX = 设计 vX = 用例 vX = 一致性矩阵 vX

### 8. 自检结论
- [ ] 全部 ✅，可交付开发段
- [ ] 存在 ❌，已列出 ❌ 项 + 处理方式

### 9. 签字
- 自检人（analyst）：____
- 复核人：____
- 日期：____
```

---

## 交付包总览（产品段步骤 11-12 填写：11 自检 + 12 打包）

```markdown
## 交付包总览

**项目**：
**产品交付日期**：
**产品负责人**：
**接收方（开发）**：

### 交付物清单
| # | 交付物 | 路径 | 版本 | 评审状态 | 签字方 |
|---|--------|------|------|---------|--------|
| 1 | 竞品分析 | knowledge/doc/{ver}/analysis/competitive-analysis.md | vX | 条件出 | — |
| 2 | PRD | knowledge/doc/{ver}/prd.md | vX | ✅ 通过 | 5/5 |
| 3 | 设计稿 | knowledge/doc/{ver}/design/*.html | vX | ✅ 通过 | 5/5 |
| 4 | 设计稿截图 | knowledge/doc/{ver}/design/screenshot-<page>.png | — | 条件出 | — |
| 5 | 测试用例 | knowledge/doc/{ver}/test-cases.md | vX | ✅ 通过 | 5/5（逐份立即评审） |
| 6 | 非功能需求 | knowledge/doc/{ver}/non-functional-requirements.md | — | — | — |
| 7 | 一致性矩阵(含验收标准) | knowledge/doc/{ver}/consistency-matrix.md | vX | ✅ 通过 | — |
| 8 | **自检报告** | **knowledge/doc/{ver}/handoff-self-check.md** | **—** | **✅ 通过** | **—** |

### 版本对齐
- PRD vX = 设计 vX = 用例 vX ✅
- 一致性矩阵版本 = vX

### 自检完成
- [ ] 8 项产品交付物(5 必出 + 3 条件出)全部通过自检
- [ ] 评审会签记录完整
- [ ] 一致性矩阵全部 ✅

### 交付备注
（特殊说明、已知风险、上下文补充等）
```

---

## 自检不合格的处理

| 自检发现 | 处理方式 |
|---------|---------|
| PRD 模糊字眼 | 返回步骤 4 补齐 + 重走 PRD 评审 |
| 设计稿违反规范 | 返回步骤 6b 补齐 + 重走设计评审 |
| 用例与 PRD 不一致 | 返回步骤 8 补齐 + 重走用例评审 + 触发强同步 |
| 三项版本号不一致 | 按一致性矩阵核对，回退到落后项 + 触发强同步 |
| 一致性矩阵 §1 验收方法与 PRD 冲突 | 返回步骤 9.5 重新填矩阵 §1 + 触发强同步 |
| 一致性矩阵 ❌ 项未处理 | 返回对应步骤（4 / 6b / 8）修改 + 触发强同步 |
| 非功能需求缺指标 | 返回步骤 10 补齐具体数字 |

---

## 强同步触发后的版本管理

强同步触发时，按以下顺序更新：

1. 确认触发源（如：PRD 评审驳回）
2. 修改源项，版本号 +1
3. 同步修改受影响的另外两项，版本号 +1
4. 按原评审选择重评该份 + 被牵连文档（不整体重走所有评审）
5. 评审通过后，重新生成一致性矩阵
6. 三项版本号重新对齐为新 vX

**禁止**：仅更新源项版本号而不重走其他评审。
**禁止**：版本号跳号（如 v1 直接变 v3）。
