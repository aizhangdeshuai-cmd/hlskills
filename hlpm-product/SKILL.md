...
**问题 3: 评审模式选择**
- 选项 A: **联合评审**(重量,3 场独立会签:PRD/设计/用例 各 1 场)
  - 适用:复杂需求 / 跨模块改动 / 多人协作 / 高风险改动
  - 流程:步骤 5 PRD 评审 → 步骤 7 设计评审 → 步骤 9 用例评审,每场会签通过后才进入下一步
  - 强同步规则:任一驳回强制重走后续所有评审
- 选项 B: **最终 3 合 1 评审**(轻量,1 场集中评审)
  - 适用:轻量需求 / 迭代优化 / bug 修复 / 单模块微调
  - 流程:跳过中间 3 场会签,在 9.5 一致性终检**之前**插入 1 场集中评审(评审 PRD/设计/用例 三方对齐)
...
|------|---------|---------------|------|---------|
| 轻量 | 涉及 | **3 合 1** | 轻量+设计+3合1 路径 | 2b/2c(竞品) / 6a.2 简化 / 中间 3 场评审合并 / 9.5 只跑 2 矩阵 |
| 轻量 | 不涉及 | **3 合 1** | 轻量无设计+3合1 路径 | 2b/2c(竞品) / 整个第二阶段 / 中间 3 场评审合并 / 9.5 只跑 2 矩阵 |
| 标准 | 涉及 | 联合评审(默认) | 完整 23 步 | (无跳过) |
| 标准 | 不涉及 | 联合评审(默认) | 标准无设计 路径 | 整个第二阶段 |
| 复杂 | 涉及 | 联合评审(强制) | 完整 23 步 + 加严 | (无跳过;评审会签需 5 方全到) |
| 复杂 | 不涉及 | 联合评审(强制) | 复杂无设计 路径 | 整个第二阶段;非功能需求加深 |

> **评审模式可被 0.5 步骤问题 3 覆盖**:
> - 复杂需求"评审模式"字段被锁定为"联合评审",**用户不可改**(风险太大)
> - 轻量/标准需求"评审模式"字段可由用户问题 3 自由选择
> - 用户选择优先于默认:轻量需求用户也可选"联合评审"(想要更严格把关)
...
5. **★ PRD 评审会签**（重量评审 1/3，阻塞点,仅"联合评审"模式执行）
   - **触发条件**:仅当 0.5 步骤评审模式选 A "联合评审" 时执行本步骤
   - **跳过条件**:选 B "3 合 1" 时,本步骤+步骤 7+步骤 9 全部跳过,跳到步骤 9a"集中评审"
...
6b. **UI/UX 设计**（`designer`"已有页面微调"产出规则）
   ...
   **"已有页面微调"设计稿产出规则**（v14 强制）：
   ...
   >
   > **目的**：让设计稿成为"活文档"——交付时附带该页面每个交互元素对应的 PRD/测试用例/状态机/一致性矩阵逻辑，**评审人员点一下 🔗 图标就能看到这条规则在哪**。

   区块 4 + 区块 5 的强制规范：

   **区块 4（逻辑说明交互层）**：
   - 在页面 `<body>` 末尾追加 `<aside class="logic-panel">` 抽屉（默认 hidden）
   - 抽屉内含标题区 + 内容区 + 关闭按钮
   - 在页面所有交互元素（按钮 / 输入框 / 列表行 / 状态标签等）后追加 `<sup class="logic-badge">?</sup>` 小图标
   - 小图标点击 → JS 读取该元素的 `data-prd` 等属性 → 从区块 5 数据里查 → 渲染到抽屉

   **区块 5（逻辑说明数据）**：
   - 在 `<head>` 末尾追加 `<script type="application/json" id="logic-data">{...}</script>`
   - 数据格式：`{ "BR-015": { "prd": "...", "tc": ["TC-045 步骤...", "TC-046 步骤..."], "state": "订单状态: 待支付/已支付/...", "matrix": "业务规则覆盖矩阵 #15 / 状态机覆盖矩阵 #3" } }`
   - **designer 必须从实际 PRD/测试用例文件读取原文片段填入**，不可编造

   **data-* 属性约定**：
   - `data-prd`：关联的 PRD 业务规则编号（如 `"BR-015"`）
   - `data-state`：关联的状态机分支（如 `"待支付|已支付"`）
   - `data-tc`：关联的测试用例编号，多个用逗号分隔（如 `"TC-045,TC-046"`）
   - `data-matrix`：关联的一致性矩阵条目（如 `"业务规则#15|状态机#3"`）
   - **缺失某属性 = 该元素无该维度逻辑**（如纯展示性 div 不挂 data-prd）

   **JS 交互规范**：
   - 内嵌 `<script>`，不依赖任何外部 JS 库（jQuery / Vue / React 全部不用）
   - 双击 HTML 即可在浏览器打开运行，不需要 build server
   - 抽屉 CSS 用 `:hover` / `.open` 类切换即可，不用动画库

   **diff 三区块内容不变**：区块 1（原页面）+ 区块 2（diff 清单）+ 区块 3（修改后）继续保留；区块 4 + 区块 5 是附加层。

   **完整示例**：见 `hlpm-product/examples/order-list-with-export-csv.html`（仓库内提供完整可运行 demo）
...
7. **★ 设计评审会签**（重量评审 2/3，阻塞点,仅"联合评审"模式执行）
   - **触发条件**:仅当 0.5 步骤评审模式选 A "联合评审" 时执行本步骤
   - **跳过条件**:选 B "3 合 1" 时,本步骤跳过,合并到步骤 9a"集中评审"
...
9. **★ 用例评审会签**（重量评审 3/3，阻塞点,仅"联合评审"模式执行）
   - **触发条件**:仅当 0.5 步骤评审模式选 A "联合评审" 时执行本步骤
   - **跳过条件**:选 B "3 合 1" 时,本步骤跳过,合并到步骤 9a"集中评审"
...
### 第三阶段.5:集中评审(仅"3 合 1"模式)

9a. **★ 集中评审(3 合 1)**(阻塞点,**仅当 0.5 评审模式选 B"3 合 1"时执行**)
   - **触发条件**:0.5 步骤评审模式选 B "3 合 1"
   - **跳过条件**:选 A "联合评审"时,本步骤跳过,中间 3 场独立评审已执行
   - **位置**:在步骤 8 用例编写完成后,步骤 9.5 一致性终检之前
   - **参与方**:`analyst` 主持 + `designer` + `test-engineer`(3 角色集中评审,免 architect/executor 角色)
   - **评审内容**(一次性覆盖 PRD/设计/用例 三方):
     1. **PRD 评审**:`analyst` 自审 6 大模块(业务/操作/数据/状态机/权限/非功能)是否齐全
     2. **设计评审**:`designer` 自审 6a.1 规范遵循 + 6a.2 保真度对齐(若涉及设计)
     3. **用例评审**:`test-engineer` 自审 三方对齐 + 三维覆盖(业务规则/状态机/权限)
   - **评审基线**:PRD v(latest) + 设计 v(latest)(条件) + 用例 v(latest)
   - **不通过处理**(强同步规则):
     - 驳回 → 修改对应文档 → 在本场集中评审中**整体重审** 1 次
     - 1 轮不通过 → 升级为"联合评审"模式(再走完整 3 场)
...
| 7 | 设计评审未会签通过 | 禁入 8 |
| 9 | 用例评审未会签通过(联合评审模式) | 禁入 9.5 |
| 9a | 集中评审未通过(3 合 1 模式) | 禁入 9.5,1 轮不通过升级为联合评审 |
