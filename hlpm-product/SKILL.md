---
name: hlpm-product
description: 多角色协作-产品段（23步）。从需求到交付物打包的全流程，覆盖加载上下文→竞品分析（含参考咨询）→PRD→设计规范检查→保真度咨询→设计→测试用例→三项一致性终检→打包交付。与 hlpm-dev 配套使用：产品交付物经重量评审会签后交接给开发。Use when 用户提出新需求且产品与开发是两个角色、需要显式交接，或调用 /hlpm-product。
---

# 多角色协作-产品段流程

> 属于 `hlskills` 技能系统。与 `hlpm-dev` 配套使用，构成"产品 + 开发"分工的标准开发流水线。
> 完整规范见 `hlskills/SKILL.md`。

---

## 🚨 角色边界铁律（v11 新增,先读再启动）

> **本节是本技能的最高约束,优先级高于下面所有"通用纪律"和 23 步流程**

### 三条不可违反的边界

1. **产品段绝不修改产品代码**
   - 任何 `.ts/.js/.py/.java/.go/.vue/.tsx/.jsx/...` 等源代码文件,**一律由 `hlpm-dev` 负责**
   - 产品段产出的所有内容都是**文档**(`.md` / 设计稿 `.html` / 配置文件)
   - 即使 PRD 写"该字段校验函数改为 XXX" —— 产品段只描述**业务行为**,**不写实现代码**

2. **代码修改唯一入口是 `hlpm-dev`**
   - 即便只是"改一行字段名" / "加一个状态判断" / "修一个正则",也必须由 `hlpm-dev` 接手
   - 产品段可以**在 PRD 里描述需求**,但**代码层面动手**必须交开发段

3. **设计稿是条件性输出,不是必出**
   - 涉及 **UI / 交互 / 页面布局 / 视觉变化** → 必须出设计稿
   - 纯后端 / 数据结构 / 接口 / 状态机 / 业务规则 / 算法变化 → **不出设计稿**
   - 启动第 6 阶段前,会用 `request_user_input` 确认本次是否涉及设计调整

### 输出范围总览

| 文档 | 必出 / 条件 | 说明 |
|------|------------|------|
| 竞品分析报告 | 条件出 | 仅新项目/品类调研时(2a 选 "无" 时可简化为 1 段说明) |
| PRD | **必出** | 任何需求都需 PRD 定义范围 |
| 设计稿 | **条件出** | 涉及 UI 才出;纯后端/接口跳过 |
| 测试用例 | **必出** | 任何需求都需测试用例对应 |
| 验收标准 | **必出** | 与 PRD 业务规则一一对应 |
| 非功能需求 | **必出** | 性能/安全/兼容指标 |
| 一致性矩阵 | **必出** | 9.5 步骤;条件出设计稿时矩阵自动少 1 列 |
| 自检报告 | **必出** | 步骤 11 输出 |

### 启动检查清单(第 0 阶段必读)

启动产品段前,确认以下 4 件事:
- [ ] 确认本技能是"产品段",**不是开发段**;代码修改请启动 `hlpm-dev`
- [ ] 确认本次需求**是否涉及 UI 变化**(决定是否出设计稿)
- [ ] 确认需求规模(决定走"完整 23 步"还是"小需求快路径")
- [ ] 确认项目已有 `.hl/memory/` 和 `docs/` 的现状

---

## 通用纪律：完成前验证

**任何阶段标记"完成"前，必须执行验证规则：**
- 识别能证明该阶段完成的验证命令，完整运行
- 读取完整输出并检查退出码，确认通过后方可声明完成
- **绝对禁止**："应该通过了"、"看起来正确"、"可能没问题"
- **铁律**：证据先于断言，无一例外

### 受阻停止纪律
- 任何步骤遇到阻塞，**立即停止**，不得猜测或强行绕过
- 向产品经理（`analyst`）报告阻塞情况
- 产品经理无法解决时，立即向用户提问
- 3 次修复/驳回尝试失败 = 质疑架构方案，升级到用户决策
- **单项评审最多驳回 3 轮**，超出升级到用户决策

### 三项强同步纪律（核心约束）

**PRD / 设计稿 / 测试用例 三项必须逻辑完全一致**：
- 任一项因评审驳回而回退修改，**强制重走后续所有评审**（不可豁免）
- 三项版本号必须对齐（PRD vX = 设计 vX = 用例 vX）才能进入下一阶段
- 一致性矩阵是 9.5 步骤的单一可信源，开发段以此为验证基准
- 任何"评审驳回导致的内容变更"都触发强同步，包括字段级调整

---

## 角色与 Agent 映射

| 角色 | Agent | 用途 |
|------|-------|------|
| 产品经理 | `analyst` / `planner` | 需求分析、PRD 编写与评审主持、流程统筹 |
| UI/UX 设计师 | `designer` | 界面设计、设计评审主持 |
| 前端开发 | `executor` | UI 可实现性评审、前端组件复用评估 |
| 后端开发 | `architect` / `executor` | 技术可行性、数据流转评审 |
| 代码审查 | `code-reviewer` | （开发段使用） |
| 测试 | `test-engineer` / `qa-tester` | 用例编写与评审、E2E 场景 |
| 验证 | `verifier` | 一致性矩阵验证、交付物自检 |
| 用户（我） | — | 关键节点确认者 |

> **⚠️ 重要：Agent 能力边界**
>
> - `analyst` 是**只读 Agent**（`disallowedTools: Write, Edit`），仅负责**分析 / 评审 / 内容生成**（以消息形式输出）
> - **实际文件写入**（PRD / 一致性矩阵 / 验收标准 / 交付物自检清单等）由**主 agent 或 `planner` 负责**
> - `designer` / `test-engineer` / `executor` 可写可改，负责对应产出物
> - 步骤说明中"X Agent 编写"= "X Agent 生成内容 + 主 agent 或调用方写入文件"
> - `verifier` 只读，负责验证和出具判定（✅/❌）

---

## 产品段开发流程（23 步）

### 第零阶段：加载项目上下文
0. **加载已有文档** — 在分析新需求之前，**先读取项目已有知识**，避免重复分析：
   - 如存在 `.hl/memory/` → Read 全部 memory 文件（project.md / techstack.md / architecture.md / conventions.md / working.md）
   - 如存在已有分析文档（竞品分析 / PRD / 技术设计 / ADR）→ Read 加载
   - 如有 `docs/design/spec.md` 或同级设计规范文件 → **记录路径**，供 6a.1 使用
   - 迭代需求场景：必须先理解项目现状，再分析新需求

### 第零阶段.5：规模判断(v11 新增,阻塞点)

0.5. **★ 需求规模与设计需求评估** — `analyst` 启动后,先用 `request_user_input` 问 2 个问题:

**问题 1: 本次需求规模**
- 选项 A: **轻量需求** (业务规则 < 3 条,只改 1 个模块/页面,预计代码 < 100 行)
- 选项 B: **标准需求** (业务规则 3-10 条,涉及 2-3 个模块)
- 选项 C: **复杂需求** (业务规则 > 10 条,涉及多模块/多角色/有非功能要求)

**问题 2: 本次是否涉及设计调整**
- 选项 A: **涉及**(UI / 交互 / 页面布局 / 视觉变化)
- 选项 B: **不涉及**(纯后端 / 数据 / 接口 / 状态机 / 业务规则 / 算法)

**根据回答决定后续路径**:

| 规模 | 涉及设计 | 路径 | 跳过步骤 |
|------|---------|------|---------|
| 轻量 | 涉及 | 轻量+设计 路径 | 2b/2c(竞品)/ 6a.2(保真度咨询简化为 1 句) / 9.5 只跑 2 矩阵 |
| 轻量 | 不涉及 | 轻量无设计 路径 | 2b/2c(竞品)/ 整个第二阶段(6/7) / 9.5 只跑 2 矩阵 |
| 标准 | 涉及 | 完整 23 步 | (无跳过) |
| 标准 | 不涉及 | 标准无设计 路径 | 整个第二阶段(6/7) |
| 复杂 | 涉及 | 完整 23 步 + 加严 | (无跳过;评审会签需 5 方全到) |
| 复杂 | 不涉及 | 复杂无设计 路径 | 整个第二阶段(6/7);非功能需求加深 |

**注意**:
- 用户可随时在后续步骤中**升级路径**(如发现原本"轻量"实际复杂)
- 升级路径时,已完成的步骤不重做,**补齐跳过的步骤**即可
- 用户可随时在后续步骤中**降级路径**(如发现"涉及设计"实际不涉及) → 跳过整个第二阶段

### 第一阶段：需求阶段

1. **需求分析** — `analyst` 基于已有文档上下文，理解并梳理新需求。
   - 如用户提供需求文档（含图片）：用 Read 工具读取文本；优先用模型原生多模态识别图片中的草图/流程图/参考截图
   - 图片内容与文字需求合并分析，形成完整需求理解

2a. **★ 竞品分析 - 询问参考竞品**（阻塞点）
   - 用 `request_user_input` 问用户：是否有参考竞品？
   - 选项：**有** / **无** / **不确定**
   - 【阻塞点】：未询问用户不得进入 2b/2c
   - 不允许"用户没回应就跳过"或"默认无参考"

2b. **竞品分析 - 读取参考竞品**（仅当 2a 选"有"时执行）
   - 用户指定具体竞品：读取其官网/产品文档/用户评价，输出参考竞品画像
   - 用户提供文件（截图/录屏/PRD）：用多模态识别内容
   - 产出：参考竞品核心功能 / 差异化优势 / 定价策略
   - 完成后进入 2c

2c. **竞品分析 - 品类调研**（hlpmnew 路径必做；hlpm 路径推荐做）
   - 识别 3-5 个直接竞品与间接竞品
   - 输出竞品分析报告：功能矩阵 / 定价 / 优劣势
   - 明确自身产品定位与差异化方向

3. **目标用户画像 / 差异化定位**（仅新项目，hlpmnew 路径）
   - 输出用户群体 / 使用场景 / 核心诉求排序
   - 差异化定位必须能"一句话说清"，否则标 🔴 风险

4. **PRD 编写**（`analyst` 生成内容 → 写入文件，6 大模块）
   - 业务逻辑：每个功能的完整业务规则，包含所有分支条件、边界情况、异常路径
   - 操作流程：每个用户操作的主流程 + 替代流程 + 异常流程
   - 数据流转：字段级定义（字段名/类型/必填/校验规则/默认值/示例值）
   - 状态机：核心实体（订单/用户/任务等）的完整状态流转图
   - 权限规则：每个功能/页面的访问权限（游客/登录用户/管理员等）
   - 非功能需求：性能指标（响应时间/QPS）、安全要求、兼容性要求
   - **不确定就问**：任何业务逻辑、数据规则、流程细节有不确定的地方，立即向用户提问
   - **禁止模糊字眼**：PRD 中不得出现"暂定""待确认""或""等"
   - **自检标准**：写完后随机抽取 3 个功能点，模拟设计师/开发/测试角色分别提问，无疑问方为合格
   - **输出基线版本 v1**，登记到 `consistency-rules.md` 的版本基线表

5. **★ PRD 评审会签**（重量评审 1/3，阻塞点）
   - 参与方：`analyst` 主持 + `architect` + `executor` + `designer` + `test-engineer`
   - 评审基线：PRD v(latest)
   - 各角色各自输出评审意见（**通过 / 有保留 / 反对**）
   - **不通过 → 返回步骤 4 → 输出 PRD v(N+1) → 触发【强同步规则】**
   - **3 轮上限**：同一项评审最多驳回 3 轮，超出升级用户决策

   **【v5 强同步规则 - PRD 评审驳回时】**
   PRD 重写后不仅需重走步骤 5 自身（直到通过），还必须：
   - 在设计稿（步骤 6b）相应位置同步修改
   - 在测试用例（步骤 8）相应位置同步修改
   - 重走步骤 7 设计评审 + 步骤 9 用例评审
   - **即便设计/用例原本已通过，也必须重走（不能豁免）**

### 第二阶段：设计阶段（v11 条件性执行:仅当 0.5 步骤选"涉及设计"时执行）

> **条件触发**:0.5 步骤问题 2 选 A "涉及 UI/交互/页面/视觉变化" 才执行本阶段
> **跳过条件**:0.5 步骤问题 2 选 B "不涉及(纯后端/数据/接口/状态机/规则/算法)" → 整个第二阶段直接跳过,跳到步骤 8 用例编写
> **本阶段产出**:仅是"设计稿 + 设计评审";不写产品代码,不调接口,不建数据库

6a.0 **设计需求确认**(v11 新增,执行本阶段前先问)
- 如未在 0.5 步骤明确,此处再问一次:本次是否涉及 UI 变化?
- 选 "不涉及" → 跳到步骤 8(本阶段 6a.1/6a.2/6b/7 全部跳过)
- 选 "涉及" → 继续

6a.1 **★ 设计规范检查**（阻塞点）
   - 读取项目目录，自动检查是否存在设计规范：
     - 检查路径（按优先级）：
       1. `docs/design/spec.md`
       2. `docs/design/SKILL.md`
       3. `docs/design/` 目录下任意 `.md` 文件
       4. 项目根目录 `DESIGN.md`
   - 找到 → **严格遵循**：字体/色彩/间距/圆角/阴影/组件样式/动效基调 一律按规范
   - 未找到 → 输出"未发现设计规范"报告，继续 6a.2
   - 【阻塞点】：必须执行目录扫描 + 输出检查报告，无论是否找到
   - 找到规范后，designer 需在设计前输出**规范遵循清单**：
     - [ ] 字体（标题/正文/代码字号字重行高）
     - [ ] 色彩（主色/辅色/强调色/中性色 + 亮/暗双套）
     - [ ] 间距系统（基础单位 + xs/sm/md/lg/xl 各级）
     - [ ] 圆角与阴影（按钮/卡片/弹窗分级）
     - [ ] 核心组件（按钮 4 态、输入框 4 态、标签/徽章/头像）
     - [ ] 动效基调（hover/过渡/缓动曲线）

6a.2 **★ 设计保真度咨询**（阻塞点）
   - 用 `request_user_input` 问用户选哪一档：
     - **低保真原型**（线框图 / 灰度布局 / 基础组件，适合内部评审、流程对齐）
     - **中保真 HTML 原型**（默认推荐：色彩 + 真实组件 + 完整交互 + 真实示例数据）
     - **高保真 HTML 原型**（像素级还原 + 全部状态 + 动画 + 真实数据，适合用户验收/对外演示）
   - 【阻塞点】：未询问用户不得进入 6b 设计
   - **保真度一经选定，整个项目统一遵循该档**，不可临时升级要求

6b. **UI/UX 设计**（`designer`）
   - 按 6a.1 规范 + 6a.2 保真度产出设计
   - 遵循 hlpm 三步决策（如果 6a.1 未找到规范）：
     1. 检查现有设计规范（已在 6a.1 完成）
     2. 是否先整理设计规范（询问用户：是否调用 `hllegacy` 第 11 步整理）
     3. 是否有参考页面（询问用户）
   - 控件复用：优先使用项目已有的前端组件库
   - 风格统一：新设计必须与当前系统风格保持一致
   - 架构统一：遵循项目已有的前端架构模式
   - 与当前系统页面逐项对比（截图对照）：字体/按钮/表格/表单/弹框/间距/分割线/图标
   - **输出基线版本 v1**，登记到 `consistency-rules.md` 的版本基线表

7. **★ 设计评审会签**（重量评审 2/3，阻塞点）
   - 参与方：`designer` 主持 + `analyst` + `executor` + `architect` + `test-engineer`
   - **评审基线**：
     - 规范遵循：必须与 6a.1 找到的规范完全一致（v4 硬性检查项）
     - 保真度：以 6a.2 选定的档为基线，不可临时升级要求
     - 内容对齐：与 PRD v(latest) 业务规则一致
   - 不通过 → 返回步骤 6b → 输出设计 v(N+1) → 触发【强同步规则】
   - 3 轮上限：同一项评审最多驳回 3 轮，超出升级用户决策

   **【v5 强同步规则 - 设计评审驳回时】**
   设计重写后不仅需重走步骤 7 自身（直到通过），还必须：
   - 检查 PRD（步骤 4）相应位置是否需要同步（设计变更是否影响业务逻辑）
   - 在测试用例（步骤 8）相应位置同步修改
   - 如 PRD 受影响 → 重走步骤 5 PRD 评审
   - 重走步骤 9 用例评审
   - **即便 PRD/用例原本已通过，也必须重走**

### 第三阶段：测试准备

> **v11 补充**:即使跳过了第二阶段(设计),本阶段仍必须执行
> 测试用例的双源对齐:有设计稿时对齐 PRD+设计;无设计稿时对齐 PRD+技术约束(架构/接口/数据)
> 一致性矩阵在"无设计"场景下自动少 1 列(无"设计页面"列)

8. **测试用例编写**（`test-engineer`，含 E2E）
   - 按 **PRD v(latest) + 设计 v(latest) 双源对齐编写**
   - 覆盖：主流程 + 替代流程 + 异常流程
   - 包含：边界值用例、权限分支用例、E2E 场景
   - 用例必须能反推 PRD 验收标准
   - **输出基线版本 v1**，登记到 `consistency-rules.md` 的版本基线表

9. **★ 用例评审会签**（重量评审 3/3，阻塞点）
   - 参与方：`test-engineer` + `analyst` 联合
   - **评审基线**：
     - 用例 v(latest) + PRD v(latest) + 设计 v(latest) 三方对齐
     - 业务规则 / 状态机 / 权限 三个维度均覆盖
   - 不通过 → 返回步骤 8 → 输出用例 v(N+1) → 触发【强同步规则】
   - 3 轮上限

   **【v5 强同步规则 - 用例评审驳回时】**
   用例重写后不仅需重走步骤 9 自身（直到通过），还必须：
   - 反查 PRD（步骤 4）验收标准是否与新用例匹配
   - 反查设计（步骤 6b）交互流程是否覆盖新用例
   - 如 PRD/设计受影响 → 重走步骤 5 PRD 评审 + 步骤 7 设计评审
   - **即便 PRD/设计原本已通过，也必须重走**

### 第四阶段：交付前终检（v5 新增）

9.5. **★ 三项一致性终检**（v5 新增，阻塞点）
   - 在所有评审通过后、进入交付前执行
   - **Agent 分工**：
     - `analyst`（只读）**提取要素**：从 PRD/设计/用例三方抽取业务规则、状态机、权限点、非功能指标
     - `analyst` **生成初稿**：输出"一致性矩阵"内容（按 `consistency-rules.md` 模板）
     - 写入文件由主 agent/调用方负责
     - `verifier`（只读）**验证一致性**：交叉比对三方覆盖情况，出具 ✅/❌ 判定
   - **提取范围**（与第 4 矩阵字段逐字对齐）：
     - PRD：业务规则 + 状态机分支 + 验收条件 + 非功能指标（**性能 / 安全 / 兼容**三类分别列）
     - 设计：页面 + 状态 + 交互流程 + 非功能 UI 实现策略（**性能 / 安全 / 兼容**三类分别列）
     - 用例：前置条件 + 步骤 + 预期结果 + 非功能验证方法（**性能 / 安全 / 兼容**三类分别列）
   - **三类非功能需求定义**：
     - **性能**：QPS / 响应时间 / FPS / 并发数
     - **安全**：加密 / 鉴权 / 脱敏 / 注入防护
     - **兼容**：浏览器 / 设备 / 系统版本
   - 三方交叉比对：每个 PRD 业务规则 → 必须有对应设计页面 + 对应用例
   - **输出【一致性矩阵】**（按 `consistency-rules.md` 模板）：
     - 业务规则覆盖矩阵
     - 状态机覆盖矩阵
     - 权限覆盖矩阵
     - 非功能需求覆盖矩阵（性能 / 安全 / 兼容性）
     - 不一致清单
   - 不一致 → 返回对应步骤（4 / 6b / 8）修改 → 触发强同步 → **重走后续所有评审**（强同步规则：不可豁免已通过的评审）
   - 9.5 返回后路径示例：
     - 不一致项在 PRD → 返回 4 → PRD 评审（5）必须重走 + 设计评审（7）必须重走 + 用例评审（9）必须重走
     - 不一致项在设计 → 返回 6b → 设计评审（7）必须重走 + 用例评审（9）必须重走
     - 不一致项在用例 → 返回 8 → 用例评审（9）必须重走
   - 【阻塞点】：一致性矩阵未生成 + 全部 ✅ 通过 → 禁入步骤 10

10. **验收标准与非功能需求打包**（`analyst` + `test-engineer` 生成 → 写入文件）
    - 验收标准：从 PRD 业务规则 + 用例反推的可验证条款
    - 非功能需求：性能 QPS/响应时间、安全、兼容性
    - 与一致性矩阵对齐

11. **交付物自检**（`analyst` 用 `handoff-package.md` 逐项核对 → 输出自检报告）
    - 5 项交付物全部勾选自检清单
    - 一致性矩阵存在且勾选完整
    - 三项版本号匹配（PRD vX = 设计 vX = 用例 vX）
    - **自检报告落盘**：写入 `docs/handoff-self-check.md`（与 `handoff-package.md` 配套，供开发段参考）
    - `analyst` 生成自检报告内容 → 主 agent/调用方写入文件

12. **打包交付**
    - 输出 5 项必交物 + 一致性矩阵 + 自检报告（v6 强化）：
      1. PRD（含版本号）
      2. 设计稿（含版本号）
      3. 测试用例（含版本号）
      4. 验收标准
      5. 非功能需求
      6. 一致性矩阵（v5 新增）
      7. **自检报告**（v6 新增：`docs/handoff-self-check.md`，步骤 11 输出，供开发段步骤 0 验证）
    - 通知开发接手（`hlpm-dev` 步骤 0）

---

## 硬性关卡汇总

| 步骤 | 关卡 | 触发后果 |
|------|------|---------|
| 2a | 未询问参考竞品 | 禁入 2b/2c |
| 5 | PRD 评审未会签通过 | 禁入 6a.1 |
| 6a.1 | 未执行目录扫描 + 输出检查报告 | 禁入 6a.2 |
| 6a.2 | 未询问保真度 | 禁入 6b |
| 6b | 找到规范后未严格遵循 | 设计稿需重做 |
| 7 | 设计评审未会签通过 | 禁入 8 |
| 9 | 用例评审未会签通过 | 禁入 9.5 |
| 9.5 | 一致性矩阵未生成 + 全部 ✅ | 禁入 10 |
| 任一驳回 | 回退修改未重走后续评审 | 强同步失败，禁入下一阶段 |

---

## 交付文档清单（产品段 6 项）

| # | 文档 | 路径 | 负责 Agent | 步骤 |
|---|------|------|-----------|------|
| 1 | 竞品分析报告 | `docs/analysis/competitive-analysis.md` | `analyst` 生成 → 写入文件 | 2b/2c |
| 2 | PRD 文档 | `docs/prd.md` | `analyst` 生成 → 写入文件 | 4 |
| 3 | 设计稿 | `docs/design/*.html` | `designer` | 6b |
| 4 | 测试用例 | `docs/test-cases.md` | `test-engineer` | 8 |
| 5 | 验收标准 | `docs/acceptance-criteria.md` | `analyst` 生成 → 写入文件 | 10 |
| 6 | 非功能需求 | `docs/non-functional-requirements.md` | `analyst` 生成 → 写入文件 | 10 |
| 7 | 一致性矩阵 | `docs/consistency-matrix.md` | `analyst` 生成 + `verifier` 验证 | 9.5 |
| 8 | **自检报告** | `docs/handoff-self-check.md` | **`analyst` 生成 → 写入文件** | **11** |

---

## 变更传播规则

当用户在"确认设计"或后续阶段提出修改时，**所有关联文档必须同步更新**：
- PRD 变更 → 设计稿 + 测试用例 + 一致性矩阵同步更新
- 设计稿变更 → PRD（如果影响业务逻辑）+ 测试用例 + 一致性矩阵同步更新
- 测试用例变更 → PRD（反查验收标准）+ 设计稿（反查交互流程）+ 一致性矩阵同步更新
- 任一变更 → 触发【强同步规则】，重走后续所有评审

产品经理（`analyst`）负责追踪，确保所有文档一致性。

---

## 与 hlpm/hlpmnew 的关系

| 维度 | hlpm / hlpmnew | hlpm-product + hlpm-dev |
|------|---------------|------------------------|
| 适用场景 | 单人 / 小团队（一人跑完全流程） | 多人协作（产品 + 开发两角色分工） |
| 评审机制 | 内部评审，analyst 汇总 | 重量评审 3 场独立会签 |
| 一致性 | 无显式机制 | 强同步 + 一致性矩阵（v5） |
| 交接 | 无显式交接 | 5 项交付物 + 一致性矩阵打包 |
| 拒收 | 无 | 开发可拒收 + 拒收原因 |

**原 `hlpm` / `hlpmnew` 一字不改，新技能作为变体并存。**

---

## 不在本技能范围

> **本技能明确不做以下事情**,如需请调用对应技能:

- ❌ **任何源代码的修改**（.ts/.js/.py/.java/.go/.vue/.tsx/.jsx/...）—— 改代码请用 `hlpm-dev`
- ❌ **数据库迁移、部署、CI/CD 配置** —— 用 `hlpm-dev` / `hldb` / `hldeploy`
- ❌ **依赖安装、构建、跑测试** —— 用 `hlpm-dev`
- ❌ **真实代码评审、安全扫描** —— 用 `hlpm-dev` 步骤 5/9
- ❌ **git 提交、PR 推送、tag 发布** —— 用 `hlpm-dev` 步骤 8/14
- ❌ **跨会话的"产品 AI + 开发 AI"自动协作**（本期不实现编排器）
- ❌ **历史版本回溯**（v5 不维护版本历史，只看当前最新）
- ❌ **自动会签工具实现**
- ❌ **自动 diff 检测**（一致性靠人工+矩阵保证）

### 本技能产出的 8 类文档清单（配套给 `hlpm-dev` 步骤 0 验证）

| # | 文档 | 路径 | 必出/条件 | 步骤 |
|---|------|------|-----------|------|
| 1 | 竞品分析报告 | `docs/analysis/competitive-analysis.md` | 条件 | 2b/2c |
| 2 | PRD 文档 | `docs/prd.md` | **必出** | 4 |
| 3 | 设计稿 | `docs/design/*.html` | 条件(0.5 选涉及) | 6b |
| 4 | 测试用例 | `docs/test-cases.md` | **必出** | 8 |
| 5 | 验收标准 | `docs/acceptance-criteria.md` | **必出** | 10 |
| 6 | 非功能需求 | `docs/non-functional-requirements.md` | **必出** | 10 |
| 7 | 一致性矩阵 | `docs/consistency-matrix.md` | **必出** | 9.5 |
| 8 | 自检报告 | `docs/handoff-self-check.md` | **必出** | 11 |

**交接给 `hlpm-dev` 的 5+ 项必交物**:
- 5 项基础: PRD / 设计稿（条件）/ 测试用例 / 验收标准 / 非功能需求
- 2 项新增（v5/v6 强化）: 一致性矩阵 / 自检报告
