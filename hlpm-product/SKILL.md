...
6a.1 **★ 设计规范检查**（阻塞点，v14 强化"硬性约束"）
   - 读取项目目录，自动检查是否存在设计规范：
     - 检查路径（按优先级）：
       ...
       4. 项目根目录 `DESIGN.md`
   - 找到 → **🚨 硬性约束（v14 新增）**：字体/色彩/间距/圆角/阴影/组件样式/动效基调 **一律按规范**，**严禁偏离**：
...
6b. **UI/UX 设计**（`designer`，v14 强化"已有页面微调"产出规则）
   ...
   ### 🚨 已有页面微调的产出规则（v14 新增，硬性约束）

   > **如果本次需求是"在已有页面上做调整"（如新增按钮 / 改文案 / 调布局），设计稿必须基于原页面 diff，不允许另起炉灶**

   **两种场景区分**（用 `request_user_input` 在 6a.0 步骤确认）：
...
7. **★ 设计评审会签**（重量评审 2/3，阻塞点,仅"联合评审"模式执行）
   ...
   - **评审基线**（v14 强化"硬性检查项"）：
     ...
     - **🚨 diff 完整性**（v14 新增,仅"已有页面微调"场景）：设计稿必须包含 3 个区块 + 每条改动有规范引用 + 改动点用 diff 标记
...
### 第三阶段.5:集中评审(v13 新增,仅"3 合 1"模式)

9a. **★ 集中评审(3 合 1)**(v13 新增,阻塞点,**仅当 0.5 评审模式选 B"3 合 1"时执行**)
