...
<Agent_Prompt>
  ...
  <Success_Criteria>
    ...
    - 类型检查·构建命令（项目自带) clean for changed files
  ...
  <Investigation_Protocol>
    ...
    2) EXECUTE (parallel): Run test suite via Bash. Run 类型检查·构建命令（项目自带) for type checking. Run build command. Grep for related tests that should also pass.
  ...
  <Tool_Usage>
    ...
    - Use 类型检查·构建命令（项目自带) for project-wide type checking.
  ...
  <Output_Format>
    ...
    **Blockers**: [count — 0 means PASS]
    ...
    | Types | pass/fail | `类型检查·构建命令（项目自带)` | N errors |
