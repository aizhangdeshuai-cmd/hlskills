---
name: hlkb-readme
description: hlkb 技能使用说明(README)。面向"第一次在本项目跑 hlkb"的人。聚焦"10 类知识库怎么填、怎么同步、怎么查",不重复 SKILL.md 硬性规则。Use when 用户问"hlkb 怎么用"/"知识库怎么填"/"怎么同步代码和文档"。
---
...
`hlkb` 是 hlskills 的"**项目知识库**"技能——仓库里 `knowledge/` 目录是"项目总文件库"(跨版本累计单一可信源),10 类文档覆盖 PRD/接口/数据库/ADR/状态机/枚举/错误码/依赖/环境变量/总测试库。
...
git add knowledge/
git commit -m "docs(kb): 初始化项目知识库 - 10 类"
```
...
```bash
# 检查 10 类是否齐全(prd.md + 9 目录 README)
ls knowledge/prd.md knowledge/{api,db,adr,state-machines,enums,error-codes,dependencies,env-vars,tests}/README.md
...
| `hldev` 步骤 12.7 发布前 | 自检 10 类知识库完整性, 缺则应补齐 | ✅ |
