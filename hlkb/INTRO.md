---
name: hlkb-intro
description: hlkb 技能说明(概览版)。5 分钟读懂"项目知识库"是什么、为什么需要、10 类怎么管。Use when 用户问"知识库是什么"/"hlkb 怎么用"/"为什么需要知识库"。
---
...
**hlkb** 是 hlskills 体系中的"**项目知识库**"——仓库里 `knowledge/` 目录是"项目总文件库"(跨版本累计单一可信源),与 `src/` 代码同 commit 维护,10 类文档(PRD / 接口 / 数据库 / ADR / 状态机 / 枚举 / 错误码 / 依赖 / 环境变量 / 总测试库)。
...
- 想知道"目录结构长啥样" → 看 [`SKILL.md`](./SKILL.md) 的"目录结构(10 类知识库)"段
