---
name: hlkb-intro
description: hlkb 技能说明(概览版)。5 分钟读懂"项目知识库"是什么、为什么需要、11 类怎么管。Use when 用户问"知识库是什么"/"hlkb 怎么用"/"为什么需要知识库"。
---
...
**hlkb** 是 hlskills 体系中的"**项目知识库**"——仓库里 `knowledge/` 目录是"项目总文件库"(跨版本累计单一可信源),与 `src/` 代码同 commit 维护,11 类文档(PRD / 术语表 / 接口 / 数据库 / ADR / 状态机 / 枚举 / 错误码 / 依赖 / 环境变量 / 总测试库)。
...
| 技能 | 触发 hlkb | 更新 | 对方已对接? |
|---|---|---|---|
| `hlpm` 版本交付自检 | 版本交付完成 | `knowledge/prd.md` / `knowledge/tests/` | ❌ 待对接 |
| `hldev` 步骤 4.5 走查 | 代码实现后 | `api/` / `db/` | ✅ |
| `hldb` 数据库迁移 | 任何 DDL | `db/` | ✅ |
| `hlapi` 接口设计 | 任何契约 | `api/` | ✅ |
| `hladr` 架构决策 | 任何决策 | `adr/` | ✅ |
| `hlbug` Bug 修复 | 涉及字段/接口 | 同步知识库 | ✅ |
| `hllegacy` 旧项目 | 知识沉淀 | 初始化整套 | ❌ 待对接 |

### 2. 11 类知识库(项目总文件库)

| 类别 | 路径 | 维护角色 | 何时更新 |
|---|---|---|---|
| 总 PRD | `prd.md` | 产品经理(`analyst`) | 版本交付后追加章节(hlpm 步骤 11) |
| 术语表 | `glossary.md` | 产品经理(`analyst`) | hlpm 步骤 1 梳理 + 步骤 4.5 同步(通用语言) |
...
| 技能 | 关系 | 对方已对接? |
|---|---|---|
| `hlpm` | 版本交付自检建议触发 master 文档追加 | ❌ 待对接 |
| `hldev` | 步骤 4.5 + 12.7 触发 API/DB 同步 + 完整性自检 | ✅ |
| `hldb` | 任何 DDL 强约束同步 db/ | ✅ |
| `hlapi` | 任何接口契约强约束同步 api/ | ✅ |
| `hladr` | 输出直接写到 adr/ | ✅ |
| `hlbug` | 涉及字段/接口变化时同步 | ✅ |
| `hllegacy` | 知识沉淀阶段建议初始化整套知识库 | ❌ 待对接 |

**核心配套**: 项目知识库 = 11 个模板 + 同步触发规则 + 自检清单。

---

## 下一步

- 想知道"目录结构长啥样" → 看 [`SKILL.md`](./SKILL.md) 的"目录结构(11 类知识库)"段
